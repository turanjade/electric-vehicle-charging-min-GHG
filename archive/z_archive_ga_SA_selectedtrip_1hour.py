...
from func_call_sa import sa_evolution_deap
...
####================================================================================================================================================#####
writefiledir = 'D:/GitHub/results/TTS_5%_2hrstop_1kWh'
tts_full_file = open('tts_toydata.csv','r')
...
#tts_sample_file = open('ttsfull_5%_processed.csv', 'w')
#tts_sample_file.write(','.join(str(i) for i in colnames)+'\n')
#for trip in tts_sample:
#	tts_sample_file.write(','.join(str(i) for i in trip)+'\n')
...
person_id = np.unique(np.array(tts_sample[:,col_personid]))#[20:25] ##travelers id, aka veh id
len_time_1min = 1440  ###total min of a day
step = 60 ###calculate per 60 min
...
for i in range(len(person_id)):
	...
	for t in range(len(persontrips[:, 1])):
		derivator = 1+math.ceil(persontrips[t, col_time].astype(float))
		erij_value = persontrips[t, col_energy].astype(float)/derivator
		erij_ini[(persontrips[t, col_starttime].astype(int)-1):(persontrips[t, col_endtime].astype(int)), i] = erij_value
erij_ini_array=[]
for i in range(len(person_id)):
...
for i in range(1,(len(erij_ini_array)+1)):
	if i%step != 0:
		...
		sum_timestep = 0
print('len of erij, len of chargingcons, len of individual', len(erij), len(chargingcons), num_bits)
